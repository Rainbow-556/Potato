...
//                    tempInstance.processorBus = PublishProcessor.create().toSerialized();
    ...
    public <T> Disposable register(Class<T> eventType, Consumer<T> consumer) {
        return toObservable(eventType).subscribe(consumer);
    }

    public <T> Disposable register(Class<T> eventType, Consumer<T> consumer, Scheduler scheduler) {
        return toObservable(eventType).observeOn(scheduler).subscribe(consumer);
    }

    private Disposable register(Class eventType, Consumer observer, Scheduler scheduler, BackpressureStrategy strategy) {
    ...
    private Disposable register(Class eventType, Consumer observer, BackpressureStrategy strategy) {
